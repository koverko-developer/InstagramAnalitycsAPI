...
function chekoutInF(req, res){
    ...
    firebase.database().ref('/users/' + userId).once('value').then(function(snapshot) {
      ...
      if(access_token === 'Anonymous'){
          ...
          getCookie(req, res, username, userId, username);
      }
    });
}
function getCookie(req,res, username, accountId, user) {
  ...
  if(req.body.count_media) count = req.body.count_media;
    ...
    .on('data', function (chunk) {
      d = chunk;
      //console.log(d);
      setCookie(d, res, count, session, accountId, dir, user);
    })
    .on('end', function () {
        ...
        if(media[k]['_params']['commentCount'] > 0) {
            ...
            const feed = new Client.Feed.MediaComments(session, media[k]['id']);
              ...
              _.each(comments, function(comment) {
                        // console.log('-------------------------------');
                        // console.log(comment['params']['account']['username'] + " - " +comment['params']['account']['fullName']);
                        // console.log(comment['params']['text']);
                        // console.log(comment['params']['account']['picture']);
